...
<form id="form-district" class="form-horizontal" method="POST">
	...
	<div class="form-group">
		<div class="col-sm-2">
			<label for="district_name"><?php __e('Name') ?></label>
		</div>
		<div class="col-sm-4">
	...
	<div class="form-group">
		<div class="col-sm-2">
			<label for="district_city_id"><?php __e('City') ?></label>
		</div>
		<div class="col-sm-4">
			<select name="district_city_id" id="district_city_id" class="form-control" required>
				<?php foreach ($cityRepository->findAll() as $city) : ?>
                <option<?php echo EBB\toAttributes(array( 'value' => $city->get('id'), 'selected' => ($city === $district->get('city')) )) ?>><?php $city->display('name') ?></option>
				<?php endforeach; ?>
			</select>
		</div>
	</div>
